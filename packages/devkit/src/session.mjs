...
/**
 * Minting a gadget development session from a developer key, so a host can
 * boot without a token pasted out of a browser tab.
 *
 ...
 * browser on the development gateway sees neither it nor the token it mints.
 */
 ...
 * is re-copy. `envVar` names the caller's own variable in the error.
 */
export function assertDeveloperKey(key, envVar) {
  if (!KEY_PATTERN.test(key ?? "")) {
    throw new Error(`${envVar} must be a personal access token (ag_mcp_…) carrying the gadget_dev.session scope.`);
  }
}

/**
 * `POST /v2/gadget-dev/sessions` authenticated by the key itself, returning
 * what the host needs and nothing else. Errors name the status and the API's
 * own message; the key never appears in one.
 */
export async function mintGadgetDevSession({ apiOrigin = DEFAULT_API_ORIGIN, developerKey, envVar = "AGENTICOS_DEV_KEY", gadgetKey, title, fetcher = fetch }) {
  const origin = normalizeOrigin(apiOrigin);
  assertDeveloperKey(developerKey, envVar);
  const response = await fetcher(`${origin}/v2/gadget-dev/sessions`, {
    method: "POST",
    headers: {
